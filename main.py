...
def register():
    if request.method == 'POST':
        ...
        sql = "INSERT INTO users (name, uname , phone, age, password, gender) VALUES (%s, %s, %s, %s, %s, %s)"
        val = (name, uname, phone, age, password, gender)
        mycursor.execute(sql, val)
        mydb.commit()
        return render_template('UserLogin.html')
...
def report_send_mail(mailid1,message,image_path):
    ...
    with open(image_path, 'rb') as f:
        img_data = f.read()
    fromaddr = "dkroman13@gmail.com"
    toaddr = "ssabariveeran@gmail.com"
    msg = MIMEMultipart()
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Alert"
    ...
    s.starttls()
    s.login(fromaddr, "omrqmqjnzbdnaodn")
    text = msg.as_string()
